...
PERIOD_BUCKETS = {
    # 早＝清晨、中＝午前、晚＝午後高峰（使用者要 15:00，不要 21:00）
    "早": (5, 6, 7, 8, 9),
    "中": (10, 11, 12, 13, 14),
    "晚": (15,),
}
...
def default_period_for_now(now=None):
    """Pick 早/中/晚 from current Taipei hour; afternoon+ maps to 晚 (15:00)."""
...
def pick_tomorrow_morning_recommendation(forecast, hours=(5, 7, 9), top_n=3, tides=None):
    ...
    for hour in hours:
        ranked = []
        for sp in SPOTS:
            slots = (forecast["spots"][sp["key"]].get("days") or {}).get(tomorrow) or []
        ...
        if not top:
            blocks.append(
                ...
                f'<div class="tm-hour-empty">此時次無資料</div></div>'
            )
            continue
        if headline is None:
        ...
        blocks.append(
            f'<div class="tm-hour"><div class="tm-hour-label">{hour:02d}:00</div>'
            f'<ol class="tm-top">{"".join(items)}</ol></div>'
        )
    html = (
        '<div class="pick-tomorrow-board">'
        '<div class="pick-tomorrow-title">明日早上推薦（玩浪 · 前三）</div>'
        f'{"".join(blocks)}'
        '</div>'
    )
...
def build_html(forecast, tides, cwa, svg, wg_fetched, cwa_fetched, pick, rec, pick_tm=None, rec_tm=None):
    if rec_tm and "<" in str(rec_tm):
        rec_tm_html = rec_tm
        pick_tm_line = f"{pick_tm}（詳見上方 05／07／09 前三）" if pick_tm else "詳見上方時次表"
    elif rec_tm:
        rec_tm_html = f'<p class="pick-tomorrow">{rec_tm}</p>'
        pick_tm_line = rec_tm
    else:
        rec_tm_html = '<p class="pick-tomorrow">明日清晨預報尚不足</p>'
...
.pick-tomorrow-title { font-weight: 700; font-size: 0.9rem; margin-bottom: 10px; color: var(--text, #e8eef7); }
.pick-tomorrow-board .tm-hour { margin-bottom: 10px; }
.pick-tomorrow-board .tm-hour:last-child { margin-bottom: 0; }
.tm-hour-label {
  font-family: "JetBrains Mono", ui-monospace, monospace;
  font-size: 0.8rem; font-weight: 700; color: #7dd3fc; margin-bottom: 4px;
}
.tm-top { margin: 0; padding-left: 0; list-style: none; }
.tm-top li {
  display: flex; flex-wrap: wrap; align-items: baseline; gap: 6px 10px;
  font-size: 0.85rem; padding: 3px 0; color: var(--text, #e8eef7);
}
.tm-rank {
...
@media (min-width: 720px) {
  ...
  .pick-tomorrow-title { grid-column: 1 / -1; margin-bottom: 0; }
}
...
<div class="wrap">
  ...
  <div class="pick-block pick-block-top">
    ...
    <ul class="pick-notes">
      ...
      <li><strong>明日早上</strong>：05／07／09 各列前三（玩浪）· 首選 {pick_tm or "—"}</li>
    </ul>
  </div>
...
{daily_rec_html}
  <div class="legend">
    ...
    <span>上方可切 早（05–09）／中（10–14）／晚（15:00）· 玩浪／玩風</span>
...
def write_tldr(forecast, tides, cwa, wg_fetched, cwa_fetched, pick, rec, pick_tm=None, rec_tm=None):
    lines = [
        ...
        "## 今天開哪？",
        f"- **今日首選 → {pick}**",
        f"- **明日早上 05／07／09 首選 → {pick_tm or '—'}**（詳見頁面表格）",
        f"- {rec}",
        "",
